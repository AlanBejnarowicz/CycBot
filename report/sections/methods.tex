...
\par The~ear actuators of CycBot were implemented as soft bending pneumatic actuators based on the~basic PneuNets actuator design introduced during Tutorial 3: Actuator Fabrication~\cite{tutorial3-actuator}. The~design was intentionally kept simple and closely followed the~tutorial example in order to ensure reliable and repeatable actuation. Each ear consists of a~silicone body cast from Ecoflex~00-30, forming an~internal air chamber that bends when inflated due to asymmetric wall thickness. The~bottom layer of the~actuator was made thicker than the~top layer, constraining expansion on one side and causing the~ear to bend downward when pressurized.

\par The~mold used for fabricating the~ear actuators was 3D-printed. Liquid silicone was mixed in a~1:1 ratio, degassed in a~vacuum chamber to remove trapped air bubbles, and poured slowly into the~mold. After pouring, the~mold was cured in an~oven at approximately 55--60$^\circ$C for approximately 20~minutes.

\par After curing, the~actuators were carefully removed from the~mold, trimmed, and fitted with pneumatic tubing. The~tubing was inserted into the~internal air channel and sealed using silicone adhesive (Sil-Poxy) to ensure an~airtight connection. The~completed ear actuators were then mounted onto the~robot's head structure.

\begin{figure}[!ht]
    \centering
    \includegraphics[height=0.9\linewidth, angle=90]{data/ears.jpeg}
    \caption{Fabricated soft pneumatic ear actuators based on the~PneuNets bending design.}
    \label{fig:ear-actuators}
\end{figure}

\par Inflation and deflation of the~ear actuators are controlled using air pumps and solenoid valves, enabling expressive ear movements used for non-verbal communication. While the~simplicity of the~actuator design ensured reliable operation, it also limited the range and precision of achievable motions, which may have affected the~recognizability of certain emotional expressions.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.9\linewidth]{data/cycbot-diagram.png}
    \caption{CycBot system diagram.}\label{fig:cycbot-diagram}
\end{figure}

\par The~other 2~pumps are responsible for inflating the~robot's ears. The~ears can be inflated independently, but as they are connected through the~solenoid valves to the~same motor shield output, they can only be deflated simultaneously. There are also no pressure sensors for the~ears, so they have to be controlled by a~feed-forward controller.
...
\par The~robot operates based on a~state machine that defines interactions with humans (see Table~\ref{tab:emotions} and Table~\ref{tab:state-changes}). The~robot starts in the~default, sad state (see Figure~\ref{fig:cycbot-sad}), where it waits for a~human interaction. Both ears are inflated to point downwards, indicating sadness. When a~single short touch is detected, the~robot transitions to the~interested state (see Figure~\ref{fig:cycbot-interested}), where it slowly swipes its head side to side, showing curiosity. If no interaction occurs for 20~seconds, the~robot returns to the~sad state.
...
\par \textbf{Hypothesis H1:} Participants will more likely rate well the~robot's emotions that involve the~ear movements (happy, sad) compared to emotions that involve only the~head movements (interested, annoyed).

\par \textbf{Measure:} Likert scale ranging from 1~(Very Poor) to 5~(Very Good).

\par \textbf{Analysis:} Mann-Whitney U-test to compare mean ratings for different emotions.

\par The~experiment involved online survey where participants watched videos of the~CycBot robot expressing different emotions (signals used in experiments are robot behaviours explained in Table~\ref{tab:emotions}):
\begin{itemize}
    \item sad,
    \item interested,
    \item happy,
    \item annoyed.
\end{itemize}
After watching each video, participants were asked to indicate \textbf{\textit{How well does the~robot simulate the~following emotion?}}, using a~5-point Likert scale ranging from 1~(Very Poor) to 5~(Very Good). Additionally, demographic data was collected from the~participants, if they approved to share it:
\begin{itemize}
    \item age,
    \item gender,
    \item prior robot interaction experience (yes/no).
\end{itemize}
The~survey was distributed via social media platforms.
